...
function SeedNotFoundDialog({ open, schemaSlug, goBack, t }: SeedNotFoundDialogProps) {
  return (
    <Dialog open={open} onOpenChange={(isOpen) => { if (!isOpen) goBack() }}>
      <DialogContent className="flex flex-col max-h-[calc(100vh-2rem)] p-0">
        <DialogHeader className="px-6 pt-6 pb-2 flex-shrink-0">
...
export function EntryEditorDialog(props: Readonly<EntryEditorDialogProps>) {
  ...
  const getSaveButtonText = () => {
    ...
    if (effectiveDraftContext) {
      return t("content.editor.saveDraft")
    }
    if (isCreate) {
      return t("common.create")
    }
    return t("content.editor.save")
  }
  ...
  if (!seed && !isSeedLoading) {
    return <SeedNotFoundDialog open={open} schemaSlug={schemaSlug} goBack={goBack} t={t} />
  }
