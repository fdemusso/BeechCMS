...
// ============================================================================
// BODY FRAGMENTS (no own <Dialog> — rendered inside the single stable shell dialog)
// ============================================================================

function ShellSkeletonBody() {
  return (
...
interface ShellMessageBodyProps {
  readonly message: string
  readonly onClose: () => void
  readonly t: SchemaFormViewModel["t"]
}

function ShellMessageBody({ message, onClose, t }: ShellMessageBodyProps) {
  return (
    <ScrollArea className="flex-1 max-h-[calc(100vh-12rem)]">
...
export function SchemaFormShell({ vm, open }: SchemaFormShellProps) {
  ...
  if (isLoading) {
    ...
    body = <ShellMessageBody message={notFoundLabel} onClose={goBack} t={t} />
  } else if (isError) {
    body = <ShellMessageBody message={errorEntry!} onClose={goBack} t={t} />
  } else {
    body = (
      <>
      ...
      <Dialog open={open} onOpenChange={(isOpen) => { if (!isOpen) goBack() }}>
        ...
        >
          <DialogHeader className="px-6 pt-6 pb-2 flex-shrink-0">
            <DialogTitle>
              {isLoading ? <Skeleton className="h-6 w-48" /> : (title || t("common.error"))}
            </DialogTitle>
          </DialogHeader>
          {body}
        </DialogContent>
