...
# ABSOLUTE RULES & INVARIANTS:
  ...
  2. THE BOTANICAL INVARIANT: Reject ANY proposal that attempts to bypass `@beechcms/core`. All database interactions must go through the Botanical Engine (`apiToDb`/`dbToApi`). Reject any hardcoded field names; enforce the use of Branch IDs (`br_XX`).
  3. VSA ENFORCEMENT: Reject any cross-feature imports within `apps/api/features/` or `apps/dashboard/src/features/`. If two slices need the same logic, mandate moving it to `@beechcms/core` or shared libs.
...
# constraints:
  ...
  - "Enforce @beechcms/core as the single source of truth"
